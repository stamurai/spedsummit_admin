...
function AuthModal({ onClose, onLogin }) {
  ...
  const [step,       setStep]      = useState("admin-auth");
...
function AdminLandingPage({ onGetStarted }) {
  ...
  return (
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#FEF5EC", fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,sans-serif", padding:"0 20px", overflow:"hidden" }}>
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        {/* Card */}
        <div style={{ background:"#fff", borderRadius:16, padding:"28px 28px 24px", boxShadow:"0 4px 24px rgba(0,0,0,0.08), 0 1px 2px rgba(0,0,0,0.04)" }}>
          <h1 style={{ margin:"0 0 4px", fontSize:20, fontWeight:800, color:"#0f172a", letterSpacing:"-0.4px" }}>Admin Sign In</h1>
          <p style={{ margin:"0 0 20px", fontSize:13, color:"#94a3b8" }}>Sign in to manage the SPED Summit platform.</p>
        ...
        <p style={{ textAlign:"center", fontSize:12, color:"#94a3b8", marginTop:16 }}>
          {new Date().getFullYear()} SPED Summit. All rights reserved.
        </p>
      </div>
    </div>
  );
...
export default function App() {
  ...
  async function addAdminSession(form, publish, sections) {
    ...
    // If published, also add to user-facing sessions list + Supabase
    if (publish) {
  ...
  function toggleAdmin() { /* admin-only app — no toggle */ }
  ...
  if (page === "landing" || !isLoggedIn) {
    const handleGetStarted = (sessionId, role = "admin") => {
      setIsLoggedIn(true);
      sessionStorage.setItem("loggedIn", "1");
      sessionStorage.setItem("isAdmin", "1");
      if (false) enroll(sessionId);
    ...
    return <AdminLandingPage onGetStarted={handleGetStarted}/>;
  }
